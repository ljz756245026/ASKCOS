...
for key, value in tqdm(encoded_rxn_dict.items()):
    rxn = rxn_le[key]
    rsmi = rxn.split('>>')[0] #reactant_smiles,
    psmi = rxn.split('>>')[1] #target,
    #Reaction condition recommendations for a rxn (SMILES) from top n NN; Returns the top n parseable conditions.
